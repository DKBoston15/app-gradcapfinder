...
addResearchParadigm: async (    
  ...
  set((state) => ({
    research_paradigms: [...state.research_paradigms, { id: data[0].id, link, title, category, project_id: selectedProject }]
  }))},

  deleteResearchParadigm: async (id) => {
      await supabase.from('research_paradigms').delete().eq('id', id);
      set((state) => ({
        research_paradigms: state.research_paradigms.filter((research_paradigm) => research_paradigm.id !== id)
      }))},

  patchResearchParadigm: async (
    id: number, title: string, link: string, category: string
  ) => {
    await supabase
    .from('research_paradigms')
    .update({
    ...
    set((state) => ({
      ...
      ? ({ ...research_paradigm, title, link, category})
      : research_paradigm
    ),
    }))},
