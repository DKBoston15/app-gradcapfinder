...
addEntry: async (    
  category: string, content: string, connectedId: string, projectId: number
) => {
  const user = supabase.auth.user();
  ...
  set((state) => ({
    feed_entries: [...state.feed_entries, { id: data[0].id, project_id: projectId, category, content, connected_id:connectedId, created_at: data[0].created_at }]
  }))},
